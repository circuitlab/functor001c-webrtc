...
export class TextureSender {
  ...
  // size を明示的に受け取るように修正
  init( canvasElement, renderer, gpuCompute, variable, velocityVariable, size ) {
    ...
    this.velocityBuffer = new Float32Array( this.size * this.size * 4 );
  ...
  update() {
    ...
    const velocityArray = this.velocityBuffer.slice( this.current, this.current + this.slicesize );
    ...
    this.pigeon.sendMsg( {
      ...
      body: {
        ...
        velocitybuffer: Array.from( velocityArray )
      }
    } );
